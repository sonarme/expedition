        ...
        requestTimer.time {
            BidRequestRules.execute(bidRequest) match {
                ...
                case None => {
                    ...
                    val imp = bidRequest.getImpList.head
                    val impId = imp.getId //we will always have one impression with a bid (based on the rules we specified in BidRequestRules)
