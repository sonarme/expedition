...
class LocationBehaviourAnalysePipe(args: Args) extends DTOPlacesInfoPipe(args) {
    ...
    def classifyPlaceType(bayestrainingmodel: String, chkinpipefileterdtime: RichPipe): RichPipe = {
        ...
        val classifiedplaces = chkinpipefileterdtime.joinWithSmaller('venName -> 'data, trainedto).project(('keyid, 'venName, 'goldenId, 'rating, 'key, 'weight)).rename('key -> 'venuetype)
        classifiedplaces
    }
}
