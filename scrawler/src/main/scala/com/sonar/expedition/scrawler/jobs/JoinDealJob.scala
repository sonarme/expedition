...
class JoinDealJob(args: Args) extends DefaultJob(args) {
    ...
    }.discard(('dealDescription, 'merchantName, 'majorCategory, 'dealImage)).map(('dealPrice, 'purchased) ->('revenue, 'dealSuccess)) {
        in: (Double, Int) => {
  ...
  .min('revenue -> 'minRevenue))*/
    ...
    val lsWithStats = ls.crossWithTiny(reveneAnalysisPipe).map(('revenue, 'revenueAverage, 'revenueStdDev) -> 'revenueBucket) {
        in: (Double, Double, Double) =>
            ...
            val bucketSize = math.sqrt(revenueStdDev)
            val bucket = math.round(math.log(revenue / bucketSize))
            bucket
    }
    ...
    /*  val lsSample = lsWithStats.limit(1000).groupAll(_.sortedTake[Int]('revenue -> 'sortedRevenue, 1000).mapStream('sortedRevenue -> 'revenueBuckets) {
          revenueSample: (Iterator[Double]) => {
              val revenueGroups = for (revenueGroup <- revenueSample.toSeq.sorted.sliding(20, 10);
                                       numItems = revenueGroup.size;
                                       average = revenueGroup.sum / numItems
              ) yield average
              revenueGroups.zipWithIndex.map(groupAverage => (groupAverage._2, groupAverage._1))
          }
      }
      ).flatMap('revenueBuckets -> 'flattenedRevenue) {
