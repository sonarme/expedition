...
class DealVenueSearchCorrelationJob(args: Args) extends DefaultJob(args) with ParseFilter {
    ...
    val raw = Tsv(outputDir + "/raw.tsv")
    //('url, 'timestamp, 'status, 'content, 'links)
    val parsed = Tsv(outputDir + "/parsed.tsv") //('url, 'timestamp, 'businessName, 'category, 'subcategory, 'rating)

    val parsedSequence = SequenceFile(outputDir + "/parsedSeq", Fields.ALL)
    //('url, 'timestamp, 'businessName, 'category, 'subcategory, 'rating)
    ...
    val parsedTuples = rawTuples
        ...
        url: String => url != null && isUrlIncluded(url)
            ...
            .map('content ->('ybusinessName, 'ycategory, 'yrating, 'ylatitude, 'ylongitude, 'yaddress, 'ycity, 'ystate, 'yzip, 'yphone, 'ypriceRange, 'yreviewCount, 'yreviews)) {
        content: String => {
    ...
    parsedTuples
            .filter('match) {
        matched: String => matched.equals("true")
    }
            .discard('match)
            .write(matchedTsv)
    ...
    parsedTuples
            .filter('match) {
        matched: String => matched.equals("maybe")
    }
            .discard('match)
