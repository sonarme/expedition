...
class AggregateMetricsJob(args: Args) extends Job(args) {
    ...
    }.leftJoinWithTiny('dealId -> 'yelpDealId, yelp.rename('dealId -> 'yelpDealId)).map(('venName, 'merchantName) ->('venName, 'merchantName)) {
        in: (String, String) => ("\"" + in._1 + "\"", "\"" + in._2 + "\"")
    }.write(Csv(metricsOut, AggregateMetricsJob.OutputFormat))
