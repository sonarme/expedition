...
class YelpCrawl(args: Args) extends DefaultJob(args) with Fetcher {
    ...
    val parsedTuples = rawTuples
            .map(('url, 'content) ->('ybusinessName, 'ycategory, 'yrating, 'ylatitude, 'ylongitude, 'yaddress, 'ycity, 'ystate, 'yzip, 'yphone, 'ypriceRange, 'yreviewCount, 'yreviews)) {
        in: (String, String) => {
            val (url, content) = in
