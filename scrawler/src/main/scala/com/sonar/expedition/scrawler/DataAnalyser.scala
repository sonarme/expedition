...
import com.sonar.expedition.scrawler.{HttpClientRest, FriendObjects, CheckinObjects}
...
class DataAnalyser(args: Args) extends Job(args) {
    ...
    }.project('key, 'fbid, 'lnid).write(TextLine(out))
...
fields : (String,List[CheckinObjects]) =>
   ...
   .write(TextLine(checkin_output)) */
    ...
    def fourSquareCall(workplace: String, locationCityLat: String, locationCityLong: String): String = {
        ...
        var responseBodyString = HttpClientRestApibkup$.fetchresponse(url)
        responseBodyString
        */
