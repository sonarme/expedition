...
class InternalAnalysis(args: Args) extends Job(args) {
    ...
    */
    ...
    val data =
        (TextLine(inputData).read.project('line).flatMap(('line) ->('id,
                'serviceType, 'jsondata)) {
            line: String => {
                line match {
    ...
    val gcount =
        employerGroupedServiceProfilePipe.getDTOProfileInfoInTuplesCity(data).project(('skey, 'currcity))
                .groupBy('currcity){
            _.size
